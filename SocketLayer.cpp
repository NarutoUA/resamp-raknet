...
#ifdef SAMPSRV
unsigned char g_pDecompressBuffer[0x4000];
int uncompress(unsigned char* out, char* in, int &len);
#else
unsigned char g_pCompressBuffer[0x8000];
void compress(unsigned char* out, const char* in, int &len);
#endif
...
//#include "ExtendedOverlappedPool.h" #RAKNET_LITE
...
int SocketLayer::RecvFrom(const SOCKET s, RakPeer *rakPeer, int *errorCode)
{
    ...
    if (len != SOCKET_ERROR)
    {
        ...
        if (!uncompress(g_pDecompressBuffer, (char*)data, len))
        {
            *errorCode = 0;
            return 1;
        }

        ProcessNetworkPacket(sa.sin_addr.s_addr, portnum, (char *)g_pDecompressBuffer, len, rakPeer);
#else
...
int SocketLayer::SendTo(SOCKET s, const char *data, int length, unsigned int binaryAddress, unsigned short port)
{
    ...
    compress(g_pCompressBuffer, data, length);
    do
    {
        // TODO - use WSASendTo which is faster.
        len = sendto(s, (char *)g_pCompressBuffer, length, 0, (const sockaddr*)& sa, sizeof(struct sockaddr_in));
    } while (len == 0);
#else
    do
    {
