...
//#include "NetworkIDGenerator.h" #RAKNET_LITE
#include "NetworkTypes.h"
#include "SHA1.h"
#include "RakSleep.h"
...
extern unsigned int g_uiCookieRand;
extern int g_iMinConnectionTime;
extern bool g_bConnCookies;
extern bool g_bLogCookies;

#ifdef SAMPSRV
void logprintf(char* format, ...);
#else
bool ProcessAuthPacket(unsigned char* data, int size, PlayerID playerId)
{
    return false;
}
#endif
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void logprintf(char* data, ...);
Packet* RakPeer::Receive(void)
{
    Packet *packet = ReceiveIgnoreRPC();
    //logprintf("PacketID = %d", packet->data[0]);
    ...
    {
        BYTE byteRPC = 0;
        if (packet->data[0] == ID_RPC)
        {
            byteRPC = packet->data[1];
        }
        else if (packet->data[0] == ID_TIMESTAMP)
        {
            byteRPC = packet->data[7];
        }
        //logprintf("Last RPC = %u", byteRPC);
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void RakPeer::AddToBanList(const char *IP, RakNetTime milliseconds/*=0*/)
{
    unsigned index;
    RakNetTime time = RakNet::GetTime();

    if (IP == 0 || IP[0] == 0 || strlen(IP) > 15 || !memcmp(IP, "127.0.0.1", 10))
        return;
...
void RakPeer::OnConnectionRequest(RemoteSystemStruct *remoteSystem, unsigned char *AESKey, bool setAESKey)
{
    ...
    //if ( AllowIncomingConnections() )
    {
        ...
        RakNet::BitStream bitStream(sizeof(unsigned char) + sizeof(unsigned short) + sizeof(unsigned int) + sizeof(unsigned short) + sizeof(PlayerIndex) + sizeof(unsigned char) + sizeof(unsigned char) + sizeof(unsigned char) + sizeof(unsigned char));
...
#ifdef  SAMPSRV
        bitStream.Write(_uiRndSrvChallenge);

        // Auth Stuff #TODO
        RakNet::BitStream bsAuth;
        BYTE byteRand = rand();
        remoteSystem->m_byteAuthTableIndex = byteRand;
        const char* szAuthSend = g_szAuthKeyTable[byteRand][0];
        BYTE byteAuthLen = strlen(szAuthSend);
        bsAuth.Write((unsigned char)ID_AUTH_KEY);
        bsAuth.Write(byteAuthLen);
        bsAuth.Write(szAuthSend, byteAuthLen);
        SendImmediate((char*)bsAuth.GetData(), bsAuth.GetNumberOfBitsUsed(), SYSTEM_PRIORITY, RELIABLE, 0, remoteSystem->playerId, false, false, RakNet::GetTime());
...
RemoteSystemStruct * RakPeer::AssignPlayerIDToRemoteSystemList(const PlayerID playerId, RemoteSystemStruct::ConnectMode connectionMode)
{
        ...
        //for ( i = 0; i < remoteSystemListSize; i++ )
    {
        if (remoteSystemList[i].isActive == false)
        {
            ...
            remoteSystem->m_byteAuthTableIndex = 0;
            remoteSystem->m_byteAuthType = 0;
            remoteSystem->m_bIsLogon = 0;
...
#endif
{
    ...
    else if ((unsigned char)(data)[0] == ID_OPEN_CONNECTION_REQUEST && length == sizeof(unsigned char) * 2 + 1)
    {
        ...
        if (g_bConnCookies)
        {
            unsigned short wCookie = *(unsigned short*)((char*)data + 1) ^ 0x6969;
            unsigned short wRandCookie = g_uiCookieRand ^ length;
            if (wCookie != wRandCookie)
            {
                if (g_bLogCookies)
                {
                ...
                unsigned char buf[4];
                buf[0] = ID_OPEN_CONNECTION_COOKIE;
                *(unsigned short*)&buf[1] = wRandCookie;
                SocketLayer::Instance()->SendTo(rakPeer->connectionSocket, (char*)&buf, 3, playerId.binaryAddress, playerId.port);
                return;
            }
        }

        // Min connection time
        static RakNetTime s_dwMinConnectionTick = 0;
        static RakNetTime s_dwMinConnectionLogTick = 0;
        RakNetTime dwTick = RakNet::GetTime();
        if (s_dwMinConnectionTick && g_iMinConnectionTime > 0
            && (dwTick - s_dwMinConnectionTick) < g_iMinConnectionTime)
        {
            if (!s_dwMinConnectionLogTick || dwTick - s_dwMinConnectionLogTick > g_iMinConnectionTime)
            {
                logprintf("Warning: Minimum time between new connections (%u) exceeded for %s. Ignoring the request.", g_iMinConnectionTime, playerId.ToString());
                s_dwMinConnectionLogTick = dwTick;
            }
            //dword_4E7FB8 = v69;
            return;
        }
        s_dwMinConnectionTick = dwTick;
#endif
        ...
        if (rss == 0 || rss->weInitiatedTheConnection == true)
        {
            ...
            {
#ifdef SAMPSRV
                logprintf("Blocking %s due to a 'server full' attack (1)", playerId.ToString());
#endif
            ...
            if (playerId.binaryAddress == s_uiLastProcessedBinaryAddr
                ...
                {
                logprintf("Blocking %s due to a 'server full' attack (2)", playerId.ToString());
                rakPeer->AddToBanList(rakPeer->PlayerIDToDottedIP(playerId), 120000);
                return;
                }
            ...
            if (rss) // If this guy is already connected remote system will be 0
            {
                ...
                packet = AllocPacket(1);
                packet->data[0] = ID_OPEN_CONNECTION_REQUEST;
                packet->bitSize = sizeof(char) * 8;
                packet->playerId = playerId;
    ...
    {
        g_wCookie = *(unsigned short*)&data[1];
        /*for (i = 0; i < rakPeer->messageHandlerList.Size(); i++)
            rakPeer->messageHandlerList[i]->OnDirectSocketReceive(data, length * 8, playerId);
            ...
            *(unsigned short*)&c[1] = (*(unsigned short*)&data[1] ^ 0x6969);
    ...
    else
    {
        ...
        if (length > 512)
        {
            ...
            rakPeer->AddToBanList(rakPeer->PlayerIDToDottedIP(playerId), 0);
#endif
            return;
        }
...
bool RakPeer::RunUpdateCycle(void)
{
    ...
    while (rcs)
    {
        ...
        if (rcs->nextRequestTime < timeMS)
        {
            ...
            char c[3];
            c[0] = ID_OPEN_CONNECTION_REQUEST;
            //*(unsigned short*)&c[1] = (unsigned short)rcs->playerId.port ^ NETCODE_OPENCONNLULZ; // dropped in 0.3.7-RC5-1
            *(unsigned short*)&c[1] = (unsigned short)g_wCookie ^ 0x6969;
            unsigned i;
            for (i = 0; i < messageHandlerList.Size(); i++)
        ...
        //for ( remoteSystemIndex = 0; remoteSystemIndex < remoteSystemListSize; ++remoteSystemIndex )
    {
        ...
        if (remoteSystemList[remoteSystemIndex].isActive)
        {
            ...
            // #TODO 0.3.7 new
            if ((remoteSystem->connectMode == RemoteSystemStruct::DISCONNECT_ASAP || remoteSystem->connectMode == RemoteSystemStruct::DISCONNECT_ASAP_SILENTLY)
                && timeMS - remoteSystem->lastReliableSend > 20000)
            {
                CloseConnectionInternal(playerId, false, true, 0);
                continue;
            }
...
#ifdef SAMPSRV
                // #TODO 037 new
                if (!remoteSystem->m_bIsLogon)
                {
                    // #TODO suspicious shit
                    RakNetTime diff = RakNet::GetTime() - remoteSystem->connectionTime;
                    if (diff > 30000)
                    {
                        logprintf("Kicking %s because they didn't logon to the game.", PlayerIDToDottedIP(playerId));
            ...
            while (bitSize > 0)
            {
                ...
                if (remoteSystem->connectMode == RemoteSystemStruct::UNVERIFIED_SENDER)
                {
                    ...
                    else if ((unsigned char)(data)[0] != ID_AUTH_KEY || !ProcessAuthPacket(data, byteSize, playerId))
                    {
                        CloseConnectionInternal(playerId, false, true, 0);
#ifdef _DO_PRINTF
                ...
                else
                {
                    ...
                    else if ((unsigned char)data[0] == ID_CONNECTED_PONG && byteSize == sizeof(unsigned char) + sizeof(RakNetTime) * 2)
                    {
                        ...
                        RakNet::BitStream inBitStream((unsigned char *)data, sizeof(unsigned char) + sizeof(RakNetTime) * 2, false); // byteSize
                        //PingStruct ps;
                        //ps.Deserialize(psBS);
                        inBitStream.IgnoreBits(8);
                        inBitStream.Read(sendPingTime);
                    ...
                    {
                        RakNet::BitStream inBitStream((unsigned char *)data, sizeof(unsigned char) + sizeof(RakNetTime), false); // byteSize
                        inBitStream.IgnoreBits(8);
                        RakNetTime sendPingTime;
                        inBitStream.Read(sendPingTime);
                    ...
                    {
                        // #TODO I AM NOT SURE ABOUT COMMENTING CODE BELOW
                        /*RakNet::BitStream inBitStream((unsigned char *)data, byteSize, false);
                        RPCIndex index;
                        inBitStream.IgnoreBits(8);
